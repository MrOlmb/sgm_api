...
**Registration Flow:**
1. ~~User signs up via Clerk~~ (DEPRECATED - Now using local authentication)
2. User completes registration form on frontend
3. **Frontend generates and uploads PDF** of the form to Cloudinary, then sends form data + PDF URL to server
4. Server stores user data and form record with PDF URL
5. Secretary reviews and approves/rejects
6. **On approval/rejection: Frontend generates final PDF version** (with signatures) and replaces the original in Cloudinary
7. **Email notifications** automatically sent for approval, rejection, or account deactivation (only to members with email addresses)

**File Upload Requirements:**
- ~~3 required photos: `id_front_photo`, `id_back_photo`, `selfie_photo`~~ (DEPRECATED)
- **New workflow**: Optional selfie photo and signature as Cloudinary URLs
- **PDF Generation**: Frontend handles PDF creation and Cloudinary upload
- **Required field**: `url_image_formulaire` - URL of the PDF generated by frontend
- All images: JPEG/PNG only, max 5MB each
- Stored in Cloudinary with proper tracking for user association
