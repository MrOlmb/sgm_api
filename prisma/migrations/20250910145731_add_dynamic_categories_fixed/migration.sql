...
-- Insert default categories based on existing enum values (if any exist)
...
SELECT 
    ...
    END as "description",
    COALESCE(
        (SELECT "id" FROM "Utilisateur" WHERE "role" = 'SECRETAIRE_GENERALE' LIMIT 1),
...
-- If no existing documents, create default categories
INSERT INTO "CategorieTexteOfficiel" ("id", "nom", "description", "cree_par", "cree_le", "modifie_le", "est_actif")
SELECT 
    gen_random_uuid()::text as "id",
    "nom",
...
FROM (VALUES 
    ...
    ('Règlement Intérieur', 'Règlement intérieur de l''association'),
    ('Procès-Verbal', 'Procès-verbaux des réunions'),
    ('Circulaire', 'Circulaires et communications'),
    ('Décision', 'Décisions et résolutions'),
    ('Autre', 'Autres documents officiels')
) AS default_categories("nom", "description")
WHERE NOT EXISTS (
    SELECT 1 FROM "TexteOfficiel" WHERE "type_document" IS NOT NULL
);
...
-- For any remaining records without category, assign to 'Autre'
UPDATE "TexteOfficiel" 
SET "id_categorie" = (
    SELECT "id" FROM "CategorieTexteOfficiel" WHERE "nom" = 'Autre' LIMIT 1
)
